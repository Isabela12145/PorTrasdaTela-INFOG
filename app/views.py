...
def recompensa(request):
    ...
    perfil = get_object_or_404(usuarios, user=request.user)

    recompensas = Recompensa.objects.all().order_by('pontos_necessarios')

    recompensas_com_status = []
    for r in recompensas:
        ja_resgatada = perfil.recompensas_resgatadas.filter(id=r.id).exists()
        disponivel = (perfil.pontos >= r.pontos_necessarios) and not ja_resgatada
        pontos_faltando = max(0, r.pontos_necessarios - perfil.pontos)
        # percentuais para barras (evita divisão por zero)
        if r.pontos_necessarios > 0:
            progress_percent = min(100, int((perfil.pontos / r.pontos_necessarios) * 100))
        else:
            progress_percent = 100

        recompensas_com_status.append({
            'recompensa': r,
            'disponivel': disponivel,
            'ja_resgatada': ja_resgatada,
            'pontos_faltando': pontos_faltando,
            'progress_percent': progress_percent,
        })

    # progresso geral (meta visual de 1000 pontos)
    META = 1000
    geral_percent = min(100, int((perfil.pontos / META) * 100)) if META > 0 else 0

    context = {
        'pontos_usuario': perfil.pontos,
        'recompensas_com_status': recompensas_com_status,
        'recompensas_resgatadas': perfil.recompensas_resgatadas.all(),
        'geral_meta': META,
        'geral_percent': geral_percent,
    }
    return render(request, "recompensa.html", context)
...
def resgatar_recompensa(request, recompensa_id):
    ...
    # já resgatada?
    if perfil.recompensas_resgatadas.filter(id=recompensa.id).exists():
        return JsonResponse({'success': False, 'error': 'Recompensa já resgatada.'})

    # pontos suficientes?
    if perfil.pontos < recompensa.pontos_necessarios:
        return JsonResponse({'success': False, 'error': 'Pontos insuficientes.'})

    # desconta e marca como resgatada
    perfil.pontos -= recompensa.pontos_necessarios
    perfil.recompensas_resgatadas.add(recompensa)
    perfil.save()

    return JsonResponse({'success': True, 'novos_pontos': perfil.pontos})
